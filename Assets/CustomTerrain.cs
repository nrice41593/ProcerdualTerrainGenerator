...
public class CustomTerrain : MonoBehaviour
{
        ...
        for (int i = 0; i < terrainData.detailPrototypes.Length; i++)
        {
            ...
            for (int y = 0; y < terrainData.detailHeight; y += detailSpacing)
            {
                for (int x = 0; x < terrainData.detailWidth; x += detailSpacing)
                {
                    ...
                    float thisNoise = Utilities.Map(Mathf.PerlinNoise(x * details[i].feather,
        ...
        for (int z = 0; z < terrainData.size.z; z += treeSpacing)
        {
            for (int x = 0; x < terrainData.size.x; x += treeSpacing)
            {
                for (int tp = 0; tp < terrainData.treePrototypes.Length; tp++)
                {
                    ...
                    float thisNoise = Utilities.Map(Mathf.PerlinNoise(x * 0.001f,
    ...
    public void Perlin()
    {
        ...
        for (int y = 0; y < terrainData.heightmapHeight; y++)
        {
            for (int x = 0; x < terrainData.heightmapWidth; x++)
            {
                heightMap[x, y] += Utilities.fBM((x + perlinOffsetX) * perlinXScale,
    ...
    public void MultiplePerlinTerrain()
    {
        ...
        for (int y = 0; y < terrainData.heightmapHeight; y++)
        {
            for (int x = 0; x < terrainData.heightmapWidth; x++)
            {
                ...
                {
                    heightMap[x, y] += Utilities.fBM((x + p.mPerlinOffsetX) * p.mPerlinXScale,
            ...
            for (int x = 0; x < width; x += squareSize)
            {
                for (int y = 0; y < width; y += squareSize)
                {
                    ...
                    heightMap[midX, midY] = (float)((heightMap[x, y] +
                                                     heightMap[cornerX, y] +
                                                     heightMap[x, cornerY] +
                                                     heightMap[cornerX, cornerY]) / 4.0f + UnityEngine.Random.Range(-height, height));
                }
            }
            squareSize = (int)(squareSize / 2.0f);
